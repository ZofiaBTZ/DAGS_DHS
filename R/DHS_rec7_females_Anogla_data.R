...
DHS_rec7_females_Angola_data <- function(data_path){
  ...
  media <- c("almost every day", "at least once a week",2,3)  # at least once per week, different codings
  ...
  label_dataset <- 'DHS 2010+, all female sample'
  ...
  DHS_indicators <-list(
   ...
   # list(var = "v633a", level = 2, label = "Exist reason \n for no sex"),
    ...
    list(var = "v744a", level = c(1,"yes", "Yes", "don't know", "Don't know", 8), label = "Wife beating \n justified "), # combined a-e
    ...
    list(var = "v754jp", level = c(1,"yes", "Yes","don't know", "Don't know", 8), label = "False \n beliefs AIDS"),
  ...
  index <- sample(c(1:nrow(df)), replace=TRUE, size = 100000, prob = df$v005/1000000) #resampling with weights to be able to include DHS weighting
