...
DHS_prepare_data <- function(data_path, var_csv){
  ...
  for (i in seq(1:length(vars))){
    ...
    for (f in seq(1:nrow(levels_var))){
      ...
      #if (length(DHS_sub[DHS_sub[,vars[i]]==str(lev), vars[i]])>4) {
        #print("str_lev")
        #print(str(lev))
        #print(str(lev_name))
      ...
      #}
      #if (length(DHS_sub[DHS_sub[,vars[i]]==lev, vars[i]] >4)){
      #  print("lev")
      #  print(lev)
      #  print(str(lev))
      #  print(str(lev_name))
  ...
  DHS_sub[which(!(DHS_sub[,'mv744b']==0 & DHS_sub[,'mv744b']==0 & DHS_sub[,'mv744c']==0 
                   & DHS_sub[,'mv744d']==0 & DHS_sub[,'mv744e']==0)),'mv744b'] <- 1 
  # wife beating never justified 

  DHS_sub[which(!(DHS_sub[,'mv754jp'] ==0 & DHS_sub[,'mv754wp']==0 
                   & DHS_sub[,'mv756'] == 1))
             ,"754jp"] <- 1 # false believes. 
  #Mosquito bites, sharing food, healthy person with AIDS
  DHS_sub[which(is.na(DHS_sub[,'mv754jp'])),'mv754jp']<- 1
...
#          'mv743a'] <-"3"
  ...
  #  list(var = "mv633a", level = 2, label = "Exist reason \n for no sex"),
    list(var = "mv731", level = c(1,2), label = "Currently \n  working"), 
    ...
    list(var = "mv754jp", level = c(2,3,4), label = "False \n beliefs AIDS"),
