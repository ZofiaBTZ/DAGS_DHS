...
library(xlsx)
write.xlsx(0, file="/Output/Males/OR_all_countries.xlsx", sheetName="empty")
...
compute_ORs_males <- function(){
MW_desc <- read.csv("../Output/Males/Output_MWMR7HFL/description.csv")
MW_analyze <- read.csv("../Output/Males/Output_MWMR7HFL/to_analyze.csv")
n <- nrow(MW_desc)
...
countries <- list.files(path = "../Output/Males/",
                        ...
                        full.names = TRUE)
options(digits=2)
my_analyze <- read.csv("../Output/Males/Output_MWMR7HFL/to_analyze.csv")
my_n <- nrow(MW_desc)
...
for (k in 1:length(countries)){
  ...
  View(ORs[,,k])
  write.xlsx(ORs[,,k], file="../Output/Males/OR_all_countriesM.xlsx", sheetName=substr(countries[k],25,26), append=TRUE, row.names=FALSE)
}
...
saveRDS(ORs, "../Output/Males/ORs.Rds")
}


################# All countries
compute_ORs_males()
readRDS("../Output/Males/ORs.Rds")
...
rownames(ORs_5) <- lab_12#colnames(MW_analyze[2:(n+1)])
c_v <- rep("", 12*28)
...
ggplot(co, aes(Var1, Var2)) + # x and y axes => Var1 and Var2
  ...
  ggtitle("Odds ratios, males,  for African countries") + 
