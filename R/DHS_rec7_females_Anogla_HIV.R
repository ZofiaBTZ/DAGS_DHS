...
DHS_rec7_females_Angola_data <- function(data_path){
  ...
  ind_MWIR61 <- merge(HIV_data, ind_MWIR61, by=c('v001', 'v002', 'v003'), all.x = FALSE, all.y=FALSE ) # leaves out the HIV males results
  ...
  # list(var = "v778", level = 2, label = "AIDS \n acceptance"), 
   ...
   list(var = "hiv03", level = c(1,2,3), label = "HIV status")
  )
  ...
  index <- sample(c(1:nrow(df)), replace=TRUE, size = 500000, prob = df$v005/1000000)
